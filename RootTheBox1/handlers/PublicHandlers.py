...
class LoginHandler(BaseHandler):
    ...
    def failed_login(self):
        ...
        self.render(
            "public/login.html",
            info=None,
            errors=["Sai tài khoản hoặc mật khẩu, vui lòng thử lại!"],
        )
...
class RegistrationHandler(BaseHandler):
    ...
    def check_regtoken(self):
        ...
        if token is not None and not token.used:
            ...
            raise ValidationError("Sai định dạng token")
    ...
    def form_validation(self):
        if (
            ...
            raise ValidationError("Sai định dạng tài khoản")
        email = self.get_argument("email", None)
        if options.require_email and (not email or not len(email) > 0):
            raise ValidationError("Bắt buộc phải nhập email")
        if (
            email
            and bool(
                re.match(
            ...
            raise ValidationError("Sai định dạng email")
        if (
            ...
            raise ValidationError("Sai định dạng tên")
        if (
            User.by_handle(self.get_argument("handle", ""), case_sensitive=False)
            is not None
        ):
            raise ValidationError("Tài khoản đã được đăng ký")
        if (
            options.require_email
            and User.by_email(self.get_argument("email", None)) is not None
        ):
            raise ValidationError("Tài khoản email đã được đăng ký")
        if self.get_argument("pass1", "") != self.get_argument("pass2", ""):
            raise ValidationError("Mật khẩu không trùng nhau")
